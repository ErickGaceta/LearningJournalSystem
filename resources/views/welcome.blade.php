...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <style>
            ...
            body.bg-light .adaptive-text,
            body.bg-light h1,
            body.bg-light a {
                color: #18181b !important;
            }

            body.bg-dark .adaptive-text,
            body.bg-dark h1,
            body.bg-dark a {
            ...
            body.bg-light .theme-switcher {
                background: rgba(255, 255, 255, 0.95);
                border: 1px solid #e4e4e7;
            }

            body.bg-dark .theme-switcher {
                background: rgba(24, 24, 27, 0.95);
                border: 1px solid #3f3f46;
            }

            @media (prefers-color-scheme: light) {
                body.bg-system .theme-switcher {
                    background: rgba(255, 255, 255, 0.95);
                    border: 1px solid #e4e4e7;
                }
            }
            ...
            body.bg-dark .theme-btn {
                background: #27272a;
                border-color: #3f3f46;
                color: #ffffff;
            }

            body.bg-dark .theme-btn:hover {
                background: #3f3f46;
                border-color: #52525b;
            }

            @media (prefers-color-scheme: light) {
                body.bg-system .theme-btn {
                    background: #fafafa;
                    border-color: #e4e4e7;
                    color: #18181b;
                }

                body.bg-system .theme-btn:hover {
                    background: #f4f4f5;
                    border-color: #d4d4d8;
                }
            }
        ...
        <script>
            // Theme management - background only
            function initTheme() {
            ...
            window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change', (e) => {
                const currentTheme = localStorage.getItem('bgTheme') || 'system';
                if (currentTheme === 'system') {
                    // Force re-render by removing and re-adding the class
                    document.body.classList.remove('bg-system');
                    setTimeout(() => {
                        document.body.classList.add('bg-system');
                    }, 10);
                }
            });
    ...
    <body class="flex p-6 lg:p-8 items-center lg:justify-center min-h-screen flex-col">
        ...
        <div class="floating-image">
            <img src="{{ asset('favicon.png') }}" alt="Floating decoration" class="w-full h-full object-contain">
        </div>
            ...
            @if (Route::has('login'))
                <nav class="flex items-center justify-end gap-4">
                    @auth
                        <a
                            href="{{ url('/dashboard') }}"
                            class="adaptive-text inline-block px-5 py-1.5 border rounded-sm text-sm leading-normal transition-all"
                        >
                            Dashboard
                        </a>
                    @else
                        <a
                            href="{{ route('login') }}"
                            class="adaptive-text inline-block px-5 py-1.5 rounded-sm text-sm leading-normal border transition-all"
                        >
                            Log in
                        </a>
